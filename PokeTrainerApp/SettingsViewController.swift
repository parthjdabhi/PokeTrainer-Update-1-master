...
class SettingsViewController: UIViewController, UITextViewDelegate, UITextFieldDelegate {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        self.ref.child("users").child(user!.uid).child("profileData").child("gameDetails").observeSingleEventOfType(.Value, withBlock: { (snapshot) in
            CommonUtils.sharedUtils.hideProgress()
            if snapshot.exists() {
                if let dictData = snapshot.valueInExportFormat() as? NSDictionary {
                    print(dictData)
                    if let t_team = dictData["type_team"] as? String
                    {
                        if t_team == self.type_team[0] {
                            self.btn_select_team(self.btn_Team_MYSTIC)
                        }
                        else if t_team == self.type_team[1] {
                            self.btn_select_team(self.btn_Team_INSTINCT)
                        }
                        else if t_team == self.type_team[2] {
                            self.btn_select_team(self.btn_Team_VALOR)
                        }
                    }
                    if let t_play = dictData["type_play"] as? String
                    {
                        if t_play == self.type_play[0] {
                            self.btn_select_play(self.btn_Play_CASUAL)
                        }
                        else if t_play == self.type_play[1] {
                            self.btn_select_play(self.btn_Play_SEMIPRO)
                        }
                        else if t_play == self.type_play[2] {
                            self.btn_select_play(self.btn_Play_DIEHARD)
                        }
                    }
                    if let t_hunt = dictData["type_hunt"] as? String
                    {
                        if t_hunt == self.type_hunt[0] {
                            self.btn_select_hunt(self.btn_Hunt_TRAINER)
                        }
                        else if t_hunt == self.type_hunt[1] {
                            self.btn_select_hunt(self.btn_Hunt_GROUP)
                        }
                    }
                }
            }
    ...
    @IBAction func updateProfile(sender: AnyObject) {
        ...
        let data = ["type_team": Selected_type_team, "type_play": Selected_type_play, "type_hunt": Selected_type_hunt]
        self.ref.child("users").child(user!.uid).child("profileData").child("gameDetails").updateChildValues(data)
